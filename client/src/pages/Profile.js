...
const Profile = () => {
  ...
  const { data: saved, isLoading: loadingSaved } = useQuery(['saved-polls'], async () => (await axios.get('/api/polls/user/saved')).data);
  const { data: trades, isLoading: loadingTrades } = useQuery(['my-trades'], async () => (await axios.get('/api/trades/user')).data);
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-soft p-6 mb-6 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-sm text-gray-600 dark:text-gray-400">Balance: ${user?.balance?.toLocaleString()}</div>
          </div>
        </div>

        {/* Saved polls */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-soft p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Saved Polls</h2>
          {loadingSaved ? (
            <LoadingSpinner />
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(saved || []).map((p) => <PollCard key={p._id} poll={p} />)}
            </div>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-soft p-6">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">My Trades</h2>
          {loadingTrades ? (
            <LoadingSpinner />
          ) : (
                ...
                <tbody>
                  {(trades?.trades || []).map((t) => (
                    <tr key={t._id} className="border-t border-gray-100 dark:border-gray-700">
                      ...
                      <td className="py-2 pr-4 text-gray-700 dark:text-gray-300">{t.poll?.title || '-'}</td>
