...
const World = () => {
  ...
  const subCategories = ['All', 'Bolivia', 'Ukraine', 'Iran', 'Middle East', 'Global Elections', 'India-Pakistan', 'Gaza', 'Israel', 'China', 'Geopolitics'];

  const { data: pollsData, isLoading, error } = useQuery(
    ['polls', 'World', selectedSubCategory, searchTerm, currentPage],
    async () => {
      const params = new URLSearchParams({ category: 'World', page: currentPage, limit: 12 });
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2">World</h1>
          <p className="text-lg text-gray-600 dark:text-gray-400">Global events and geopolitics</p>
        </div>
      </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1">
              <form onSubmit={handleSearch} className="relative">
                <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search world polls..." className="input pl-10 pr-4 w-full" />
