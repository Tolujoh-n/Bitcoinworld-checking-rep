...
const CRYPTOS = [
  ...
  { name: 'Cardano', logo: 'https://cryptologos.cc/logos/cardano-ada-logo.png?v=032' },
  { name: 'BNB', logo: 'https://cryptologos.cc/logos/bnb-bnb-logo.png?v=032' },
  { name: 'Chainlink', logo: 'https://cryptologos.cc/logos/chainlink-link-logo.png?v=032' },
...
const Crypto = () => {
  ...
  const params = useMemo(() => {
    const p = new URLSearchParams({ category: 'Crypto', page: currentPage, limit: 12, sort: 'createdAt', order: 'desc' });
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2">Crypto</h1>
          <p className="text-lg text-gray-600 dark:text-gray-400">Markets on major cryptocurrencies</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <aside className="lg:col-span-1 space-y-6">
            ...
            <div className="space-y-2">
              {[
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded text-sm border ${timeframe === t.key ? 'border-primary-400 bg-primary-50 dark:bg-primary-950/40 text-gray-900 dark:text-gray-100' : 'border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300'}`}
                >
                  <span>{t.label}</span>
                  <span className="text-xs opacity-70">{pollsData?.pagination?.total || pollsData?.polls?.length || 0}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-soft p-4">
            <div className="font-semibold mb-3 text-gray-700 dark:text-gray-300">Crypto</div>
            <div className="space-y-2 max-h-[420px] overflow-y-auto pr-2">
              <button
                onClick={() => { setSelectedCrypto('All'); setCurrentPage(1); }}
                className={`w-full flex items-center gap-2 px-3 py-2 rounded text-sm border ${selectedCrypto === 'All' ? 'border-primary-400 bg-primary-50 dark:bg-primary-950/40 text-gray-900 dark:text-gray-100' : 'border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300'}`}
              >
              ...
              {CRYPTOS.map((c) => (
                <button
                  key={c.name}
                  onClick={() => { setSelectedCrypto(c.name); setCurrentPage(1); }}
                  className={`w-full flex items-center gap-2 px-3 py-2 rounded text-sm border ${selectedCrypto === c.name ? 'border-primary-400 bg-primary-50 dark:bg-primary-950/40 text-gray-900 dark:text-gray-100' : 'border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300'}`}
                >
                  <img src={c.logo} alt={c.name} className="w-5 h-5" />
                  <span>{c.name}</span>
                </button>
              ))}
            </div>
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {pollsData?.polls?.map((p) => (
                  <PollCard key={p._id} poll={{ ...p, cryptoName: p.cryptoName || selectedCrypto }} />
                ))}
              </div>
