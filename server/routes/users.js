...
router.get('/portfolio', auth, async (req, res) => {
  try {
    ...
    const activeTrades = await Trade.find({
      user: req.user._id,
      status: 'completed'
    }).populate('poll', 'title category image endDate isResolved winningOption');
  ...
  try {
    ...
    res.json({
      ...
      pagination: {
        ...
        totalPages: Math.ceil(total / parseInt(limit)),
        total,
        hasNext: (parseInt(page) - 1) * parseInt(limit) + user.savedPolls.length < total,
        hasPrev: parseInt(page) > 1
      }
    });
  } catch (error) {
...
router.get('/stats', auth, async (req, res) => {
  try {
    ...
    const totalTrades = await Trade.countDocuments({ user: req.user._id, status: 'completed' });
    ...
    const activePolls = await Poll.countDocuments({ createdBy: req.user._id, isActive: true });
